...
public interface UserInfoRepository extends JpaRepository<UserInfo, Integer> {
    ...
    @Query(value = "SELECT id, username, firstname, lastname, email,password " + "FROM user_info WHERE username= :USERNAME ", nativeQuery = true)
    Optional<UserInfo> findByUserName(@Param("USERNAME") String USERNAME);

    @Query(value = "SELECT id, username, firstname, lastname,email,password " + "FROM user_info WHERE email=:EMAIL ", nativeQuery = true)
